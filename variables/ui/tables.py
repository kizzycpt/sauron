from variables.Ether import Network, NetInfo
import socket
...
class Table():
    ...
    alerts_tbl = Table(title="\n[!] Alerts This Run [!]",
            title_style="blue",
            style="blue",
            show_lines=True)
        
    offline_devices_tbl = Table(title="\n[!] Offline Devices (this run) [!]",
            title_style="blue", 
            style="blue", 
            show_lines=True)

    change_tbl = Table(title="\n[!] Changes Since Last Run [!]",
            title_style="blue",
            style="blue",
            show_lines=True)

    inventory_tbl = Table(title="\n[!] Inventory [!]",
                 title_style="blue",
                 style="blue",
                 show_lines=True)
    ...
    def build_network_results_table(net_info: dict, gw_mac: str | None = None) -> Table:
        ...
        tbl.add_row("Local IP", NetInfo("local_ip", "N/A"))
        tbl_row("Default Gateway", NetInfo.get("gateway", "N/A"))
